...
pub const Expression = union(enum) {
    ...
    pub fn printStr(self: *Expression, writer: anytype) !void {
        switch (self.*) {
            ...
            .function => |*function| try function.printStr(writer),
            .call => |*call| try call.printStr(writer),
        }
    }
};
...
pub const Block = struct {
    ...
    pub fn printStr(self: *Block, writer: anytype) @TypeOf(writer).Error!void {
        try writer.print("{{ ", .{});
        for (self.statements.items) |*stmt| {
            try stmt.printStr(writer);
        }
...
pub const If = struct {
    ...
    pub fn printStr(self: *If, writer: anytype) @TypeOf(writer).Error!void {
        try writer.print("if ", .{});
...
pub const Call = struct {
    ...
    pub fn printStr(self: *Call, writer: anytype) @TypeOf(writer).Error!void {
...
pub const PrefixExpression = struct {
    ...
    pub fn printStr(self: PrefixExpression, writer: anytype) @TypeOf(writer).Error!void {
        try writer.print("(", .{});
        try writer.print("{s}", .{self.operator.str()});
...
pub const InfixExpression = struct {
    ...
    pub fn printStr(self: InfixExpression, writer: anytype) @TypeOf(writer).Error!void {
        try writer.print("(", .{});
